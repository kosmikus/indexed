...
deqt : {I O : Set} {r : Indexed I} (C : Code I O)
    → (r ⇒ r) → (⟦ C ⟧ r ⇒ ⟦ C ⟧ r)
deqt Z f o () y
deqt U f o tt tt = just refl
deqt (I i) f o x y = f i x y
deqt (X g F) f o x y = deqt F f (g o) x y
deqt (Y g F) f o x y = deqt F (f ∘ g) o x y
...
deqt {r = r} (EP C D e) f o x y with e r o

deqt (EP C D e) f o x y | ep with _≃_.iso₁ ep {x} | _≃_.iso₁ ep {y} | deqt C f o (_≃_.from ep x) (_≃_.from ep y)
deqt (EP C D e) f o x y | ep | just p₁ | just p₂ | just p = just (trans≡ (sym≡ p₁) (trans≡ (cong≡ (_≃_.to ep) p) p₂))
deqt (EP C D e) f o x y | ep | _ | _ | _ = nothing
